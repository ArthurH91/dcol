...
from distance_derivatives import dist, cp, ddist_dt
...
class TestComparisonDistOpt(unittest.TestCase):
    ...
    def setup_random_config(cls):
        ...
        cls.cp1_hppfcl_without_robot, cls.cp2_hppfcl_without_robot = cp_hppfcl(
            cls.shape1, c1_SE3, cls.shape2, c2_SE3
        )
        cls.dist_hppfcl_without_robot = dist_hppfcl(
            cls.shape1, c1_SE3, cls.shape2, c2_SE3
        )
        ...
        add_sphere_to_viewer(
            cls.viz,
            "cp1_hppfcl_without_robot",
            1.5e-2,
            cls.cp1_hppfcl_without_robot,
            color=1000,
        )  # BLUE
        add_sphere_to_viewer(
            cls.viz,
            "cp2_hppfcl_without_robot",
            1.5e-2,
            cls.cp2_hppfcl_without_robot,
            color=1000,
        )  # BLUE
        add_sphere_to_viewer(
            cls.viz, "cp1_opt", 1.5e-2, cls.cp1_opt_val, color=2396745
        )  # GREEN
        add_sphere_to_viewer(
            cls.viz, "cp2_opt", 1.5e-2, cls.cp2_opt_val, color=2396745
        )  # GREEN
        add_sphere_to_viewer(
            cls.viz, "cp1_opt_with_robot", 1.5e-2, cls.cp1_with_robot, color=100000
        )  # ?
        add_sphere_to_viewer(
            cls.viz, "cp2_opt_with_robot", 1.5e-2, cls.cp2_with_robot, color=100000
        )  # ?
        cls.viz.display(q)

    def test_cp1_random_config_hppfcl_opt(cls):
    ...
    def test_cp2_random_config_hppfcl_opt(cls):
        cls.assertAlmostEqual(
    ...
    def test_cp1_random_config_hppfcl_robot(cls):
        cls.assertAlmostEqual(
            np.linalg.norm(cls.cp1_hppfcl_without_robot - cls.cp1_hppfcl_without_robot),
            0,
            places=4,
            msg=f"The closest point 1 computed from GJK ({cls.cp1_hppfcl_without_robot})is not the same as the one computed with the robot ({cls.cp1_with_robot}).",
        )

    def test_cp2_random_config_hppfcl_robot(cls):
        cls.assertAlmostEqual(
            np.linalg.norm(cls.cp2_hppfcl_without_robot - cls.cp2_hppfcl_without_robot),
            0,
            places=4,
